        ...
        <div class="flex-1 flex border-gray-200 bg-white md:rounded-r-3xl p-4">
          <div class="flex-1 px-4 py-2 text-sm truncate justify-between h-full flex flex-col space-y-4">
            <p class="flex justify-between">
              <NuxtLink class="text-gray-900 font-bold hover:text-gray-600 text-2xl" :to="{ name: 'locations-title', params: { title: listing.attributes.title.toLowerCase().replace(' ','-') }}">{{ listing.attributes.title }}</NuxtLink>
